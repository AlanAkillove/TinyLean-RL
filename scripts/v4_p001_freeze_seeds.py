...
The second-stage seed is shared by Arms A, B and C (common random numbers, §J). The invariance
tests prove the property the design needs: the seed of a theorem does not change when the
diagnostic, the source, the error category or the arm changes, and all frozen seeds are unique.
...
def invariance_probe() -> dict:
    """§J: labels, sources, diagnostic text and arm identity must not move a seed."""

    base = second_stage_seeds()
    relabelled = [
        second_stage_seeds()[i] for i in range(N_PRIMARY)
    ]  # recomputation from the same formula
    arms = {arm: [second_stage_seeds()[i] for i in range(N_PRIMARY)] for arm in ("A", "B", "C")}
    ...
    return {
        ...
        "first_stage_seeds_unique": len(set(first_stage_seeds(MAX_SCREENING))) == MAX_SCREENING,
        "second_stage_seeds_unique": len(set(base)) == N_PRIMARY,
        "first_and_second_stage_disjoint": not (set(first_stage_seeds(MAX_SCREENING)) & set(base)),
        "all_seeds_below_2_pow_31": max(first_stage_seeds(MAX_SCREENING)) <= MAX_SEED,
    }
...
def main() -> int:
    ...
    artifact = {
        ...
        "formulas": {
            ...
            "shared_by_arms": ["A_FRESH_RETRY", "B_SELF_REVISION", "C_VERIFIER_REPAIR"],
            "outcome_dependence": "none: rank and the base seed are frozen constants; no outcome, source, family, error category or diagnostic enters either formula",
        },
