...
# an independent RNG seed. Config identical to P3-B/E019 except the seed knobs
# audited in docs/seed_control_audit.md:
#   data.seed=20260918 (dataloader shuffle)
#   actor_rollout_ref.rollout.seed=20260918 (vLLM engine RNG)
...
echo "[E020 seed2] fresh 60-step replication, seed=$SEED2"
echo "  config: frozen P3-B recipe; only data.seed and rollout.seed differ"
...
exec bash "$SCRIPT_DIR/run_p3_pilot.sh" --steps 60 --n 8 --dir "$RUN_DIR" --skip-prewarm \
  data.seed="$SEED2" actor_rollout_ref.rollout.seed="$SEED2" \
