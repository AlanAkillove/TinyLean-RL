...
- **无人值守批次结果（2026-09-18）**：**Seed2 复制（E020）✓**——60/60、全期 IGR 0.150 vs seed1 0.158，“先升后落”形态双种子复现、峰值位置种子相关；**温度×组大小机制实验（E020-M）✓**——n 4→8 单向增加 informative 组（T=0.6 +6/−0，p=0.031），候选成功率四条件几乎不变（0.244–0.250）；**Qwen3-Base 冷启动诊断（E021）✓**——IGR 0.0156、5 步 smoke 零混合组 → reward-dead 边界；**Seed3（E022）中止暂停**（外部非项目 GPU 任务 + oomd，按协议不重启）；**final holdout 与 MiniF2F 暂停**，恢复条件见 `experiments/manifests/m1_seed_replication.yaml`。唯一可用种子旋钮为 `+data.seed`（审计：`docs/seed_control_audit.md`）。
