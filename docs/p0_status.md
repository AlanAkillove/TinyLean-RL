...
- 下一步：M1 final holdout **已封存**（`experiments/manifests/m1_final_holdout.json`：excluded 763 / eligible 6857 / selected 128，seed 20260918；两次独立运行选择一致）→ **E023 双机评估**（fly90=seed1+seed3 / fly122=θ0+seed2；128×4、T=1.0、top_p=1.0、max 4096、同种子表；多种子分析器 `holdout_multiseed_analyze.py` 仅 fly90 运行）→ MiniF2F；不自动 step100、不扩集、不重抽。
