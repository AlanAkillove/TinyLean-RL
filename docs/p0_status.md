...
- 下一步：M1 final holdout **已封存**（`experiments/manifests/m1_final_holdout.json`：excluded 763 / eligible 6857 / selected 128，seed 20260918；两次独立运行选择一致）→ **E023（host policy amended：primary = 四模型全在 fly122，冻结于 fly122 seed1/seed3 结果出现前）**：primary 已完成 θ0 **120/512**、seed2 **113/512**（adj ✓ credit 0）；seed1/seed3 待 fly122 补跑；fly90 seed1 评估（**119/512**）重定位为 **cross-device sensitivity run（保留、不入 primary）**。四份 primary 齐全后 → canonical analyzer（仅 fly90）→ MiniF2F；不自动 step100、不扩集、不重抽。
